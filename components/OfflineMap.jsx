...
export default function OfflineMapWithLocation() {
    ...
    const init = async () => {
      ...
      map.on('click', (e) => {
        ...
        if (isRoutingRef.current) {
          setRoutePoints((prev) => {
            const updated = [...prev, [lng, lat]];
            if (updated.length === 2) setIsRouting(false);
            return updated;
          });
        } else if (isMarkingRef.current) {
          setClickedMarkers((prev) => [...prev, [lng, lat]]);
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      <button onClick={() => { setRoutePoints([]); setIsRouting(true); alert('نقطه مبدا و مقصد را روی نقشه انتخاب کنید.'); }} style={{ position: 'absolute', bottom: '140px', left: '15px', zIndex: 1000 }}>🧭 مسیریابی</button>
      <button onClick={locateUser} style={{ position: 'absolute', bottom: '80px', left: '15px', zIndex: 1000 }}>📍 مکان من</button>
      <button onClick={() => { setIsMarkingEnabled((prev) => !prev); setIsRouting(false); }} style={{ position: 'absolute', bottom: '200px', left: '15px', zIndex: 1000 }}>📌 {isMarkingEnabled ? 'در حال ثبت نقطه' : 'ثبت نقطه'}</button>
      <button onClick={handleShareLocation} style={{ position: 'absolute', bottom: '20px', left: '15px', zIndex: 1000 }}>📤 اعلام موقعیت</button>
      ...
      <AnimatePresence>
        {showConfirmModal && confirmCoords && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.2)', zIndex: 2000 }} onClick={() => setShowConfirmModal(false)}>
            <motion.div
              drag="y"
              dragConstraints={{ top: 0, bottom: 0 }}
              ...
              exit={{ y: 300 }}
              transition={{ type: 'spring', stiffness: 300, damping: 30 }}
              style={{ background: 'rgba(255,255,255,0.7)', borderRadius: '20px 20px 0 0', padding: '20px', maxWidth: '500px', margin: '0 auto', bottom: 0, position: 'absolute', left: 0, right: 0 }}
            ...
            >
              ...
              <div style={{ marginTop: '16px', display: 'flex', gap: '10px', justifyContent: 'center' }}>
                <button onClick={() => { window.location.href = `tel:*123*${confirmCoords[0]}*${confirmCoords[1]}#`; setShowConfirmModal(false); }}>☎️ شماره‌گیری</button>
